...
To obtain comprehensive Anti-CRISPR annotations, we incorporate multiple tools to identify anti-CRISPR proteins from the PhageScope database. We first collected the anti-CRIPSR proteins from **Anti-CRISPRdb** [22] to create a local nucleotide database and aligned phage protein sequences with the database via **mmseqs** [17], with stringent criteria for reliable alignment (identity > 80% and coverage > 40%) employed to identify anti-CRISPR proteins.  
